...
class DetectorObjetivos(abc.ABC):
    @abc.abstractmethod
    async def procesarImagenes(plantilla, imagen_poligono) -> float:
        # Carga de imagenes
        """
        ...
        o rgba.
        """
        plantilla = cv2.imread("CerberusAI\objetivo.png")
        ancho1 = plantilla.shape[1]
        largo1 = plantilla.shape[0]
        dimensiones = (ancho1, largo1)

        imagen_poligono = cv2.imread("CerberusAI\objetivo1.jpg")
        ...
        imagen_poligono_escala_grises = cv2.cvtColor(imagen_poligono, cv2.COLOR_BGR2GRAY)
        ...
        print(f"Similutud de las imagenes {similitud}%")

        if similitud < 60.0 or similitud == 100:
            print("Error! La imagen ingresada no es válida o se encuentra sin impactos.")
        else:
            ...
            for c in contornos:
                ...
                if area > 40:
                    ...
                    cv2.rectangle(imagen_poligono, (x, y), (x + w, y + h), (36, 255, 12), 2)
        return similitud